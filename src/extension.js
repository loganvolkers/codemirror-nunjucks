...
import {tokenizeTag} from './tokenizer';


/**
 * 
 * Nunjucks extension for CodeMirror
 * 
 * Relies on `nunjucks` mode existing for code highlighting
 * 
 */
export default class NunjucksExtension{
    
    constructor({showModal = noopModal, tagDefinitions = noopTags, editor, render = noopRender}){
    ...
    init(){
      ...
      this.editor.on('cursorActivity', debouncedRefreshFn);
      this.editor.on('viewportChange', debouncedRefreshFn);
    ...
    async _highlightNunjucksTags (render) {
      ...
      for (let lineNo = vp.from; lineNo < vp.to; lineNo++) {
        const line = this.editor.getLineTokens(lineNo);
        ...
        for (const tok of newTokens) {
          ...
          const mark = this.editor.markText(start, end, {
          ...
          const deferredUpdate = () => {
            this._updateElementText(renderString, mark, tok.string);
          };
          ...
          el.addEventListener('click', async () => {
            ...
            this.showModal({
              ...
              onDone: template => {
                ...
                if (pos) {
                  const {from, to} = pos;
                  this.editor.replaceRange(template, from, to);
                } else {
          ...
          el.addEventListener('dragstart', e => {
            ...
            // Add some listeners
            this.editor.on('beforeChange', beforeChangeCb);
            this.editor.on('drop', dropCb);
          });
          ...
          el.addEventListener('dragend', e => {
            ...
            if (droppedInSameEditor) {
              ...
              this.editor.replaceRange('', from, to, '+dnd');
            }
    
            // Remove listeners we added
            this.editor.off('beforeChange', beforeChangeCb);
            this.editor.off('drop', dropCb);
          });
    ...
    async _updateElementText (render, mark, text) {
      ...
      try {
        ...
        if (tagMatch) {
          ...
          const tagDefinition = (await this.getTagDefinitions()).find(d => d.name === tagData.name);
...
function noopModal({template,onDone}){
    console.log("Show modal", template, onDone);
}
...
function noopRender(text, contextCacheKey = null){
  return text;
}